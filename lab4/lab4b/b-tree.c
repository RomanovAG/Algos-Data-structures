...
void timingInOrderTraversal(Node *ptr, int *list_len)
{
    ...
    timingInOrderTraversal(ptr->next[0], list_len);

    if (ptr->key[0] != NULL)
        (*list_len)++;

    timingInOrderTraversal(ptr->next[1], list_len);

    if (ptr->key[1] != NULL)
        (*list_len)++;

    timingInOrderTraversal(ptr->next[2], list_len);

    if (ptr->key[2] != NULL)
        (*list_len)++;

    timingInOrderTraversal(ptr->next[3], list_len);
...
int bInsertNonFull(Node *node, const char *key, const unsigned info);
...
int bInsertNonFull(Node *node, const char *key, const unsigned info)
{
    ...
    ptr->info[i] = malloc(sizeof (unsigned));
    *ptr->info[i] = info;
    ptr->key_num++;
    return 0;
}
...
int printTree(const Node *ptr, int space)
{
    ...
    printTree(ptr->next[3], space + 8);
    if (ptr->key[2] != NULL)
    {
        for (int i = 0; i < space; i++) printf(" ");
        printf("%s\n", ptr->key[2]);
    }
    printTree(ptr->next[2], space + 8);
    if (ptr->key[1] != NULL)
    {
        for (int i = 0; i < space; i++) printf(" ");
        printf("%s\n", ptr->key[1]);
    }
    printTree(ptr->next[1], space + 8);
    if (ptr->key[0] != NULL)
    {
        for (int i = 0; i < space; i++) printf(" ");
        printf("%s\n", ptr->key[0]);
    }
    printTree(ptr->next[0], space + 8);
    return 0;
}
